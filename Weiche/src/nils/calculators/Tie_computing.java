...
public class Tie_computing {
	...
	public static void berechnen(Weiche w) {
		List<Tie> weichenSchwellenband = weichenSchwellenband();
		for (int i = 0; i < weichenSchwellenband.size(); i++) {
			weichenSchwellenband.get(i).getRepresentation().setType("turnout");
		}
		w.getTie_list().addAll(weichenSchwellenband);
		...
		double beginn_kurve = beginn_gerade / Weiche.getRadius();
	...
	public static ArrayList<Cube> geradesSchwellenband(double start, double end) {
		ArrayList<Cube> ausgabe = new ArrayList<>();
	...
	public static List<Tie> weichenSchwellenband() {
		...
		List<Tie> ausgabe = new ArrayList<>();
