...
public abstract class VectorUtil {
    ...
    public static ArrayList<Point> intersArray(Curve c1, ArrayList<Curve> c2) {
        ArrayList<Point> ausgabe = new ArrayList<>();
        for (int i = 0; i < c2.size(); i++) {
            ausgabe.addAll(intersectionLL(c1, c2.get(i)));
        }
        return ausgabe;
    }

    public static ArrayList<Point> intersectionLL(Curve c1, Curve c2) {
        ArrayList<Point> ausgabe = new ArrayList<>();
    ...
    public static ArrayList<Point> intersectionLS(Line l, Line s) {
        ...
        double _P1P = VectorUtil.distPP(s.getp1(), p);
        if (_P1P2 >= _P1P)
            ausgabe.add(p);
        return ausgabe;
    }
    ...
    public static double distPL(Point p, Line l) {
        Point a = l.getp1();
        Point b = l.getDir();
        return VectorUtil.crossP(VectorUtil.subtract(p, a), b).len() / b.len();
    }
    ...
    public static Point middle(Point p1, Point p2) {
        return VectorUtil.multiply(VectorUtil.add(p1, p2), .5);
    }

    public static double getAngle(Point a, Point b) {
        return Math.acos(VectorUtil.scalar(a, b) / (a.len() * b.len()));
    }
